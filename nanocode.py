...
class Claude:
    ...
    def think(self, conversation):
        ...
        print("payload", payload) # Debug: check the payload before sending
        response = requests.post(self.url, headers=header, json=payload, timeout=120)
        response.raise_for_status()
        return self._parse_response(response.json()['content'])


class ToolCall:
    """ A tool invocation request form the brain """
